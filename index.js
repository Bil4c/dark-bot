...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Aguarde um pouco...Caso não obtenha, Tente novamente ⌛',
				...
				only: {
					...
					ownerG: '⚡ Este comando só pode ser usado por proprietários do grupo! ⚡',
					ownerB: '⚡ Este comando só pode ser usado pelo proprietário do bot! ⚡',
					admin: '⚡ Este comando só pode ser usado por administradores do grupo (Seu membro comum🖕)! ⚡',
					Badmin: '⚡ Este comando só pode ser usado quando o bot se torna administrador! ⚡'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["558598669655@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`⚡ Falha ao converter ${type} em adesivos⚡`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie imagens com legenda ${prefix}sticker ou a tag de imagem que foi enviada`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual é o código de linguagem ?', text, {quoted: mek})
				...
				case 'hora':
					now = new Date
                    reply("Hoje é " + now.getDay() + ", " + now.getDate() + " de " + now.getMonth() + " de " + now.getFullYear() )
					break	
				case 'clearall':
					...
					reply('Sucesso em apagar todo o chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é você?')
				...
				case 'add':
					...
					if (args.length < 1) return reply('Quem deseja adicionar?')
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o alvo que você quer remover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, emissão :\n'
						...
						mentions(`Pedido recebido, emissão : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o modo simi neste grupo ✅')
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Qual a tag alvo que você deseja clonar?')
					...
					try {
						...
						mentions(`Foto de perfil atualizada com sucesso usando a foto de perfil de @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Falha om')
					}
					break
